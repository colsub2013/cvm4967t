...
{node.style.color=color;}}}});$iTXT.ui.Hook.CLICKMODE={CLICKABLE:0,NOT_CLICKABLE:1,TOOLTIP_VISIBLE:2,DELAYED:3};$iTXT.subscribe('$iTXT:contextualizer:drop',function(){$iTXT.SUPPRESS_PBF=true;});$iTXT.subscribe('$iTXT:initialiser:load:complete',VM._.bind($iTXT.ui.HookManager._onNoHook,$iTXT.ui.HookManager));$iTXT.subscribe('$iTXT:initialiser:load:complete',function(){var triggerpoint=$iTXT.glob.params.getInt('js.daw.trigger',30);var storedheight=document.body.offsetHeight;function onScrollEnd(){if(storedheight*(triggerpoint/100)<window.pageYOffset&&!VM.DAW.loaded){var url=$iTXT.glob.params.get('js.daw.url',"http://images.intellitxt.com/ast/js/vm/jslib/resources/daw.js");$iTXT.core.Util.dropScript(url+"?refresh="+(new Date()).getTime());VM.DAW.loaded=true;}}
var scrollFinish;function checkForScrollEnd(){clearTimeout(scrollFinish);scrollFinish=setTimeout(function(){onScrollEnd();},100);}
if(!!$iTXT.glob.params.get('js.daw.partners',false)&&!!$iTXT.glob.params.get('js.daw.statschannel',false)&&!$iTXT.core.Browser.isSmartphoneOrTablet()){var daw_al=$iTXT.data.AdLogger.getInstance();var ad={params:new $iTXT.data.Param($iTXT.glob.params,{})};var dam_init=(new Date()).getTime();daw_al.monitor(ad,{mt:139,mv:'daw_enabled',mv2:0});function onDawOpen(){var now=(new Date()).getTime()-dam_init;daw_al.monitor(ad,{mt:139,mv:'daw_load',mv2:now});}
...
VM.DAW={loaded:false,partners:$iTXT.glob.params.get('js.daw.partners'),pvu:$iTXT.glob.params.parse('${pvu}'),iframe:$iTXT.glob.params.get('js.daw.iframe','http://adservices02.picadmedia.com/d/pbf/v1'),ipid:$iTXT.glob.params.get('js.daw.statschannel'),page:window.location.href,onopen:onDawOpen,onclose:onDawClose,on_no_ad:onNoAd};$iTXT.core.$(window).itxtSubscribe('scroll',checkForScrollEnd);}
