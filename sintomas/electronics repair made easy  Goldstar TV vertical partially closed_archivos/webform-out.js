        ...
        var addTestObj = function(itemId) {
            ...
            wfs[itemId].test = function()
            {
                ...
                wfs[this.id].preview = function(wid)
                {
                    ...
                    var ai = $webform('​<div id="ajax_indicator" style="display: none; width: 100%; height: 100%; position: fixed; top: 0; z-index: 1000; left: 0; z-index:10; background:#000000;"></div>').css({opacity: 0});
                    ai.appendTo($body);
                ...
                var tt =
                    {
                        ...
                        html: ['<div  style="-webkit-border-radius: 5px; -moz-border-radius: 5px; border-radius: 5px; border:1px solid #f55e5e; background:#f4f5f5; padding:8px;"><div style="z-index:1; background:#f4f5f5;" class="dialog errorMsg"><p class="jsToolTipTextBox" style="padding:0; margin:0; font-size:12px; color:#555555;"> </p></div></div> <div class="arrPoint" style="background:url(http://getresponse.com/images/core/dialogs/error_arrow_up.gif) no-repeat 0 0; height:13px; left:20px; position:absolute; top:13px; width:23px; z-index:-1;"></div>'],
